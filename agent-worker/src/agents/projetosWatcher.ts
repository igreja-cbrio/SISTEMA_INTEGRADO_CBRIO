...
import { projetosReadTools, projetosReadToolNames } from "../tools/projetosRead.js";
import { createProjetosProposeTools } from "../tools/projetosPropose.js";

const AGENT_TYPE = "module_projetos_watcher";
const MAX_TURNS = parseInt(process.env.AGENT_MAX_TURNS || "20", 10);
const MODEL = process.env.PROJETOS_MODEL || "claude-sonnet-4-6";
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const SKILL_PATH = path.join(__dirname, "..", "skills", "projetos", "SKILL.md");
function loadSkill() { try { return fs.readFileSync(SKILL_PATH, "utf8"); } catch { return "Watcher Projetos"; } }
const PROMPT = `Execute o ciclo Projetos.\n1. listar_projetos_atrasados\n2. listar_projetos_sem_lider\n3. listar_projetos_sem_update (dias=30)\n4. verificar + propor. Max 8.`;
...
export async function runProjetosWatcher(opts: { triggeredBy?: string | null; config?: Record<string, unknown> } = {}) {
  ...
  try {
    const { tools, toolNames } = createProjetosProposeTools(runId);
    const server = createSdkMcpServer({ name: "projetos", version: "0.1.0", tools: [...projetosReadTools, ...tools] });
    const stream = query({ prompt: PROMPT, options: { model: MODEL, mcpServers: { projetos: server }, allowedTools: [...projetosReadToolNames, ...toolNames], systemPrompt: loadSkill(), maxTurns: MAX_TURNS, permissionMode: "default" } });
    for await (const msg of stream) {
      if (msg.type === "assistant" && msg.message?.content) {
        ...
        for (const t of tu) { if (typeof t.name === "string" && t.name.startsWith("mcp__projetos__propor_")) n++; }
