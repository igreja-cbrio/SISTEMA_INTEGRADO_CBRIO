...
import { gruposReadTools, gruposReadToolNames } from "../tools/gruposRead.js";
import { createGruposProposeTools } from "../tools/gruposPropose.js";

const AGENT_TYPE = "module_grupos_watcher";
const MAX_TURNS = parseInt(process.env.AGENT_MAX_TURNS || "20", 10);
const MODEL = process.env.GRUPOS_MODEL || "claude-sonnet-4-6";
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const SKILL_PATH = path.join(__dirname, "..", "skills", "grupos", "SKILL.md");
function loadSkill() { try { return fs.readFileSync(SKILL_PATH, "utf8"); } catch { return "Watcher Grupos"; } }
const PROMPT = `Execute o ciclo Grupos.\n1. listar_grupos_sem_encontro (dias=30)\n2. listar_grupos_sem_lider\n3. verificar_proposta_existente + propor. Max 8.`;
...
export async function runGruposWatcher(opts: { triggeredBy?: string | null; config?: Record<string, unknown> } = {}) {
  ...
  try {
    const { tools, toolNames } = createGruposProposeTools(runId);
    const server = createSdkMcpServer({ name: "grupos", version: "0.1.0", tools: [...gruposReadTools, ...tools] });
    const stream = query({ prompt: PROMPT, options: { model: MODEL, mcpServers: { grupos: server }, allowedTools: [...gruposReadToolNames, ...toolNames], systemPrompt: loadSkill(), maxTurns: MAX_TURNS, permissionMode: "default" } });
    for await (const msg of stream) {
      if (msg.type === "assistant" && msg.message?.content) {
        ...
        for (const t of tu) { if (typeof t.name === "string" && t.name.startsWith("mcp__grupos__propor_")) n++; }
