...
import { eventosReadTools, eventosReadToolNames } from "../tools/eventosRead.js";
import { createEventosProposeTools } from "../tools/eventosPropose.js";

const AGENT_TYPE = "module_eventos_watcher";
const MAX_TURNS = parseInt(process.env.AGENT_MAX_TURNS || "20", 10);
const MODEL = process.env.EVENTOS_MODEL || "claude-sonnet-4-6";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const SKILL_PATH = path.join(__dirname, "..", "skills", "eventos", "SKILL.md");

function loadSkill(): string {
  try { return fs.readFileSync(SKILL_PATH, "utf8"); }
  catch { return "Watcher Eventos · alerta sobre tarefas atrasadas e eventos com baixa preparacao."; }
}

const USER_PROMPT = `
Execute o ciclo de monitoramento de Eventos agora.

Roteiro:
1. listar_eventos_proximos (dias=30)
2. listar_tarefas_atrasadas
3. listar_tarefas_sem_responsavel (apenas_criticas=true) primeiro · depois sem o filtro se precisar
4. Pra eventos a 1-14d, buscar_evento_completude · alertar se pct < 70

Max 10 propostas. Prioridade: eventos < 7d + criticos > tarefas atrasadas > orfas.

Sumario: eventos proximos, atrasos detectados, propostas enviadas, foco.
`.trim();
...
export async function runEventosWatcher(opts: {
  ...
  try {
    const { tools, toolNames } = createEventosProposeTools(runId);
    const server = createSdkMcpServer({ name: "eventos", version: "0.1.0", tools: [...eventosReadTools, ...tools] });
    const stream = query({
      prompt: USER_PROMPT,
      options: {
        model: MODEL,
        mcpServers: { eventos: server },
        allowedTools: [...eventosReadToolNames, ...toolNames],
    ...
    for await (const msg of stream) {
      if (msg.type === "assistant" && msg.message?.content) {
        ...
        for (const tu of toolUseBlocks) {
          if (typeof tu.name === "string" && tu.name.startsWith("mcp__eventos__propor_")) alertasPropostos++;
        }
