...
import { patrimonioReadTools, patrimonioReadToolNames } from "../tools/patrimonioRead.js";
import { createPatrimonioProposeTools } from "../tools/patrimonioPropose.js";

const AGENT_TYPE = "module_patrimonio_watcher";
const MAX_TURNS = parseInt(process.env.AGENT_MAX_TURNS || "20", 10);
const MODEL = process.env.PATRIMONIO_MODEL || "claude-sonnet-4-6";
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const SKILL_PATH = path.join(__dirname, "..", "skills", "patrimonio", "SKILL.md");

function loadSkill(): string {
  try { return fs.readFileSync(SKILL_PATH, "utf8"); }
  catch { return "Watcher Patrimonio · bens em manutencao longa, emprestados sem retorno, cadastro incompleto."; }
}

const USER_PROMPT = `
Execute o ciclo de monitoramento do Patrimonio.

Roteiro:
1. listar_bens_por_status (status='manutencao', dias_minimos_no_status=30)
2. listar_bens_por_status (status='emprestado', dias_minimos_no_status=60)
3. listar_bens_incompletos (valor_minimo=500, dias_min_cadastro=30)
4. Pra cada caso, propor com numeros concretos.

Max 8 propostas. Inclua nome do bem e valor (R$).
`.trim();
...
export async function runPatrimonioWatcher(opts: { triggeredBy?: string | null; config?: Record<string, unknown> } = {}): Promise<R> {
  ...
  try {
    const { tools, toolNames } = createPatrimonioProposeTools(runId);
    const server = createSdkMcpServer({ name: "patrimonio", version: "0.1.0", tools: [...patrimonioReadTools, ...tools] });
    const stream = query({
      prompt: USER_PROMPT,
      options: { model: MODEL, mcpServers: { patrimonio: server }, allowedTools: [...patrimonioReadToolNames, ...toolNames], systemPrompt: loadSkill(), maxTurns: MAX_TURNS, permissionMode: "default" },
    });
    for await (const msg of stream) {
      if (msg.type === "assistant" && msg.message?.content) {
        ...
        for (const t of tu) { if (typeof t.name === "string" && t.name.startsWith("mcp__patrimonio__propor_")) n++; }
