...
concluídas, planos abandonados) vive em **`docs/Codex-LEGADO.md`** — que NÃO é
...
`.Codex/skills/llm-council/`). Fluxo de 3 estágios: conselheiros (subagentes com
...
Para qualquer feature/fix/refactor solicitado pelo usuário, Codex está
autorizado a executar o ciclo completo **até produção** sem perguntar a cada
etapa:

1. Implementar em uma branch de feature (`Codex/<descrição>`).
...
- Evitar criar arquivos `.md` novos a menos que o usuário peça
  explicitamente (exceto este `AGENTS.md`).
...
(PRs #586 → #642). Qualquer sessão futura do Codex DEVE seguir estas
...
TODAS com **Opus 4.8** (`Codex-opus-4-8` · melhor visão) e cria uma **compra
...
fluxo vive só em `/solicitacoes`). PR `Codex/logistica-remove-solic-modal-fix`:
...
# Estado atual dos módulos (condensado · histórico completo em docs/Codex-LEGADO.md)
...
- **2 personas** (`services/whatsappParser.js` · Codex Haiku): número
...
4. **Processamento** → `backend/services/cerebroProcessor.js` baixa o
   ...
   **Codex Haiku** classificar e resumir (JSON estruturado)
...
- **Modelo**: usar `Codex-haiku-4-5-20251001` (barato e rápido)
...
├── _relatorios-ia/    ← Relatórios gerados pelo Codex
...
- **Nunca alterar o frontmatter** das notas sem manter todos os
  campos obrigatórios
- **Nunca salvar nota sem resumo** — se o Codex não conseguir
...
pesos, dashboard, perguntas pendentes) em `docs/Codex-LEGADO.md`; só
