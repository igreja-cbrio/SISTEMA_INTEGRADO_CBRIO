...
-- e da 20260716150000 · identidade_pendencias):
...
--      SEMENTE de CPF como PENDÊNCIA humana (o portal cativo é a fonte mais
--      suja do sistema — CPF de terceiro DV-válido digitado pra pegar
--      internet não pode virar identidade permanente sem revisão; o par
--      inequívoco + DV pré-filtram a fila) + DV no auto-visitante + correção
--      contínua de vínculo morto/divergente com gate de nome.
--   3. Backfill one-time: repoint dos vínculos divergentes (resíduo das
--      fusões de julho) + geração inicial das pendências de semente.
-- ============================================================================
...
BEGIN
  ...
  --     CPF passa a pertencer ao sobrevivente), MAS só com NOME compatível —
  --     o mesmo argumento que protege o vínculo vivo ("o vínculo pode estar
  --     certo e o CPF do portal errado — ex.: esposa digitou o CPF do
  --     marido") vale pro morto: sem o gate, a fusão do cadastro da esposa
  --     mandaria o vínculo dela pro MARIDO. Vínculo VIVO com CPF divergente
  --     NÃO é re-apontado → pendência humana (4c).
  UPDATE public.wifi_visitantes v
   ...
   WHERE lig.id = v.membro_id
     ...
     AND public.fn_identidade_nomes_compativeis(dono.nome, v.nome);
  ...
  -- 5b) SEMENTE como PENDÊNCIA humana: membro vivo SEM CPF vinculado a wifi
  --     com CPF DV-válido → NÃO grava direto (o portal cativo é a fonte mais
  --     suja do sistema: a esposa digita o CPF do marido pra pegar internet e
  --     todos os gates automáticos passam — DV válido, par inequívoco, CPF
  --     "livre"; sem nascimento no wifi, não há como conferir ownership).
  --     Vira pendência cpf_para_confirmar (fila do módulo Entradas). Os
  --     gates de qualidade continuam como pré-filtro: NOME compatível
  --     (vínculos legados por telefone-sozinho podem estar errados), par
  ...
  -- 6) visitante automático · CPF DV-válido com ≥2 cultos distintos e sem
  --    membro. A guarda anti-duplicata compara NORMALIZADO entre os VIVOS
  --    (alinhada ao índice uniq_mem_membros_cpf_ativo, que é sobre a
  --    expressão digits-only): comparar m.cpf CRU deixava um CPF mascarado
  --    residual invisível → 23505 no INSERT. E o passo inteiro tem handler
  --    de unique_violation (corrida/resíduo não pode derrubar o cron).
  BEGIN
    WITH pessoas AS (
      SELECT v.cpf_norm,
    ...
    novos AS (
      ...
      WHERE NOT EXISTS (
        SELECT 1 FROM public.mem_membros m
         WHERE m.deleted_at IS NULL
           AND regexp_replace(COALESCE(m.cpf,''),'\D','','g') = p.cpf_norm
      )
      RETURNING id, cpf
    )
    UPDATE public.wifi_visitantes v
  ...
  EXCEPTION WHEN unique_violation THEN
    v_criados := 0;
    RAISE WARNING 'fn_wifi_processar_vinculos passo 6: unique_violation (%) — criação de visitantes pulada nesta execução; o passo 7 religa os já existentes', SQLERRM;
  END;
  ...
  RETURN jsonb_build_object(
    ...
    'sementes_cpf_pendencia', v_semeados
  );
END;
$$;

COMMENT ON FUNCTION public.fn_wifi_processar_vinculos() IS
  'Vincula wifi_visitantes a mem_membros. v2 (20260716170000): telefone exige NOME compatível (nunca telefone sozinho) · CPF DV-válido do wifi pra membro sem CPF vira pendência cpf_para_confirmar (fila humana — o portal cativo não pode gravar identidade sozinho) · corrige vínculo morto com gate de nome (incompatível vira pendência) · DV + guarda normalizada + handler 23505 no auto-visitante.';

-- ----------------------------------------------------------------------------
-- 3. Backfill one-time (roda a v2 uma vez · o passo 4b faz o repoint dos
--    vínculos mortos com nome compatível e o 5b gera as pendências iniciais
--    de semente de CPF pra fila humana)
-- ----------------------------------------------------------------------------
DO $$
DECLARE r jsonb;
BEGIN
