...
-- Estrutura completa: graus, critérios, benefícios, avaliações, progressões
-- Seed da planilha PCS 2026 v5 (22 graus, 6 critérios, 10 benefícios, 43 colab.)
-- ─────────────────────────────────────────────────────────────────────────

BEGIN;

-- ═════════════════════════════════════════════════════════════════════════
-- 1. PCS_GRAUS · estrutura de faixas salariais (G1 a G22)
-- ═════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.pcs_graus (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  codigo          TEXT NOT NULL UNIQUE,          -- 'G1' ... 'G22'
  ordem           INT  NOT NULL UNIQUE,          -- 1..22 (para ordenação)
  nivel           TEXT NOT NULL,                 -- 'Auxiliar', 'Diretor I', etc.
  ...
  variacao_pct    TEXT,                          -- 'âncora', '+30%', '+10%'
  amplitude_pct   NUMERIC(5,2) DEFAULT 50,
  area_atuacao    TEXT,
  pontos_min      INT,                           -- ponto mínimo para enquadramento
  pontos_max      INT,                           -- ponto máximo
  observacao      TEXT,
...
-- ═════════════════════════════════════════════════════════════════════════
-- 2. PCS_CRITERIOS · 6 fatores de avaliação de cargo
-- ═════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.pcs_criterios (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  codigo      TEXT NOT NULL UNIQUE,    -- 'formacao', 'experiencia', etc.
  nome        TEXT NOT NULL,
  descricao   TEXT,
  peso        NUMERIC(4,3) NOT NULL,   -- 0.15, 0.20, etc.
  pontos_min  INT NOT NULL,            -- 15, 20, etc.
  pontos_max  INT NOT NULL,            -- 75, 100, etc.
  ordem       INT NOT NULL,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ═════════════════════════════════════════════════════════════════════════
-- 3. PCS_NIVEIS_CRITERIO · descrições dos 5 níveis por critério (30 rows)
-- ═════════════════════════════════════════════════════════════════════════
...
SELECT c.id, n.nivel, n.descricao FROM public.pcs_criterios c, (VALUES
  ('formacao', 1, 'Fund. Completo'),
  ('formacao', 2, 'Ensino Médio'),
  ('formacao', 3, 'Técnico / Sup. Cursando'),
  ('formacao', 4, 'Graduação'),
  ('formacao', 5, 'Pós / MBA / Mestrado'),
  ('experiencia', 1, 'Sem experiência'),
  ('experiencia', 2, 'Até 1 ano'),
  ('experiencia', 3, '1 a 3 anos'),
  ('experiencia', 4, '3 a 7 anos'),
  ('experiencia', 5, 'Mais de 7 anos'),
  ('complexidade', 1, 'Rotineiras e simples'),
  ('complexidade', 2, 'Variadas, procedimentos definidos'),
  ('complexidade', 3, 'Análise e decisão parcial'),
  ('complexidade', 4, 'Processos complexos'),
  ('complexidade', 5, 'Alta complexidade estratégica'),
  ('responsabilidade', 1, 'Individual / mínima'),
  ('responsabilidade', 2, 'Processos simples'),
  ('responsabilidade', 3, 'Coordena terceiros'),
  ('responsabilidade', 4, 'Responsável por área'),
  ('responsabilidade', 5, 'Resultados institucionais'),
  ('lideranca', 1, 'Sem liderança'),
  ('lideranca', 2, 'Orientação eventual'),
  ('lideranca', 3, 'Liderança informal'),
  ('lideranca', 4, 'Gestão formal de equipe'),
  ('lideranca', 5, 'Gestão de múltiplas equipes'),
  ('competencias', 1, 'Conhecimentos básicos'),
  ('competencias', 2, 'Ferramentas básicas'),
  ('competencias', 3, 'Técnica intermediária'),
  ('competencias', 4, 'Especialista na área'),
...
-- ═════════════════════════════════════════════════════════════════════════
-- 4. PCS_BENEFICIOS · catálogo de 10 benefícios
-- ═════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.pcs_beneficios (
  id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  codigo             TEXT NOT NULL UNIQUE,
  nome               TEXT NOT NULL,
  valor_referencia   TEXT,                        -- 'R$ 700 - R$ 1.050'
  vinculos_elegiveis TEXT[] NOT NULL DEFAULT '{}',-- ['CLT', 'PJ', 'PJ+', 'PREBENDA']
  criterio           TEXT,                        -- observação livre
...
-- ═════════════════════════════════════════════════════════════════════════
-- 5. PCS_BENEFICIO_GRAU · matriz elegibilidade por grau
--    Status: 'sim' | 'condicional' | 'nao'
-- ═════════════════════════════════════════════════════════════════════════
...
-- ═════════════════════════════════════════════════════════════════════════
-- 6. RH_FUNCIONARIOS · novas colunas para PCS
-- ═════════════════════════════════════════════════════════════════════════
...
-- ═════════════════════════════════════════════════════════════════════════
-- 7. RH_AVALIACOES · ciclo 360° de avaliação de desempenho
-- ═════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.rh_avaliacoes (
  ...
  ciclo_periodo       TEXT NOT NULL DEFAULT 'anual',  -- 'anual' | 'semestral'
  -- Fase 1: metas (Jan-Fev)
  metas               TEXT,
  metas_definidas_em  TIMESTAMPTZ,
  -- Fase 2: autoavaliação (Dez)
  autoavaliacao_pts   NUMERIC(5,2),                   -- soma ponderada 0-5
  autoavaliacao_em    TIMESTAMPTZ,
  autoavaliacao_obs   TEXT,
  -- Fase 3: avaliação do líder (Dez)
  lider_id            UUID,                           -- profile do líder
  lider_pts           NUMERIC(5,2),
  lider_avaliado_em   TIMESTAMPTZ,
  lider_obs           TEXT,
  -- Fase 4: calibração (Jan ano+1)
  calibracao_pts      NUMERIC(5,2),
  calibracao_em       TIMESTAMPTZ,
  calibracao_obs      TEXT,
  -- Resultado final
  pontuacao_final     NUMERIC(5,2),                   -- 0-5
  pontuacao_pcs       INT,                            -- 100-500 (pra mapear em grau)
  grau_sugerido_id    UUID REFERENCES public.pcs_graus(id),
  status              TEXT NOT NULL DEFAULT 'metas_pendentes',  -- metas_pendentes | em_andamento | autoavaliada | avaliada_lider | calibrada | concluida | cancelada
  created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- ═════════════════════════════════════════════════════════════════════════
-- 8. RH_AVALIACAO_FATORES · notas por critério dentro de cada avaliação
-- ═════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.rh_avaliacao_fatores (
  ...
  nivel           INT  NOT NULL CHECK (nivel BETWEEN 1 AND 5),
  pontos          NUMERIC(6,2),  -- nivel × peso × 20 (transforma para escala 100-500)
  observacao      TEXT,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- ═════════════════════════════════════════════════════════════════════════
-- 9. PCS_PROGRESSOES · histórico de mudanças salariais e de grau
-- ═════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.pcs_progressoes (
  ...
  motivo               TEXT,
  observacao           TEXT,
  aprovado_por         UUID,        -- profile do aprovador
  aprovado_por_nome    TEXT,
  data_efetivacao      DATE NOT NULL DEFAULT current_date,
  reajuste_coletivo_id UUID,        -- FK opcional
  created_at           TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_by           UUID
);
...
-- ═════════════════════════════════════════════════════════════════════════
-- 10. PCS_REAJUSTES_COLETIVOS · controle de reajustes anuais
-- ═════════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.pcs_reajustes_coletivos (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ano               INT NOT NULL,
  percentual        NUMERIC(7,3) NOT NULL,    -- 4.5 = 4,5%
  indice_referencia TEXT,                     -- 'INPC', 'IPCA', 'manual'
...
-- ═════════════════════════════════════════════════════════════════════════
-- 11. VIEW vw_pcs_aderencia · compa-ratio em tempo real
-- ═════════════════════════════════════════════════════════════════════════
...
SELECT
  ...
  g.faixa_ref,
  g.faixa_max,
  -- Para PJ+ usa remuneracao_bruta; demais usam salario
  COALESCE(
  ...
  -- Custo da correção (delta mensal)
  CASE
    WHEN g.id IS NULL THEN NULL
...
-- ═════════════════════════════════════════════════════════════════════════
-- 13. SEED · 43 colaboradores ativos · só insere os que ainda não existem
...
-- Para os que já existem mas estão sem grau, tenta enquadrar pelo match de nome
...
SET grau_id = g.id,
    ...
    END
FROM (VALUES
  ('Alba Maia Armelau','G1','CLT',1908.00),
  ('Maria Jane Pinto Rodrigues','G1','CLT',1908.00),
  ('Marcelo Silva Rosa Heredia da Cunha','G2','CLT',2500.00),
  ('Pery Magalhães Santos Casé','G2','PJ',2120.00),
  ('Natasha Mauerberg Litwinczuk de Faria','G3','PJ',2120.00),
  ('Elionardo Alves Rodrigues','G3','PJ',2500.00),
  ('Arthur Cecconi','G3','PJ',2120.00),
  ('Andre Teixeira Rocha','G4','PJ',3000.00),
  ('Francisco Jose Henrique de Sousa','G4','CLT',3625.91),
  ('Milena Espinelo de Castro Rochet','G4','PJ',2120.00),
  ('Diego Assis','G4','PJ',2120.00),
  ('Luzia Peron Ramos','G4','PJ',1595.16),
  ('Erivelton Gomes Lima','G4','CLT',3185.41),
  ('Leonardo Pinto Ferreira','G4','CLT',3185.41),
  ('Nicolle Algaves Litwinczuk','G4','PJ',2650.00),
  ('Cauã Pedreti da Cruz','G6','PJ',2120.00),
  ('Letícia Carvalho Baldner','G6','PJ',2120.00),
  ('Alexandra Correa Xavier','G7','CLT',2359.56),
  ('Filipe dos Santos Costa Carmet','G8','PJ',5618.00),
  ('Marcos Paulo Domingues de Almeida','G8','PJ',3364.00),
  ('Matheus Ribeiro Toscano','G8','PJ',3364.00),
  ('Sonia Cristina Barreto Litwinczuk','G8','CLT',13887.32),
  ('Lorena Pariz Leonardo Queres','G9','PJ',2600.00),
  ('Allan Santana da Silva','G9','PJ',4000.00),
  ('Fatima do Rosário Garcia Pereira','G10','PJ',5000.00),
  ('Pedro Fernandes Mendes','G10','PJ',4500.00),
  ('Pedro Paiva','G10','PJ',4000.00),
  ('Yago Coelho Torres','G10','PJ',4000.00),
  ('Alda Lorena Cellos Andrade','G10','PJ',5618.00),
  ('Jessica Salviano','G10','PJ',5618.00),
  ('Jose Ribamar dos Inocentes França','G10','PJ',7087.50),
  ('Renata Cristina Martins Bispo','G10','PJ',5618.00),
  ('Amaury de Araújo Junior','G11','PJ',4300.00),
  ('David Silva da Conceição','G11','CLT',10397.16),
  ('Juliana Carneiro Leão Ramos','G12','CLT',7498.90),
  ('Mariane Gaia','G12','PJ',5618.00),
  ('Wesley Barros Ramos','G12','PREBENDA',9588.16),
  ('Marcilio Nelio Paiva de Souza','G12','PJ',14720.00),
  ('Pedro Paulo Menezes','G14','PJ+',7500.00),
  ('Arthur Serpa','G15','PJ+',15000.00),
