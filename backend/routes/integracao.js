...
const { gerarAgendaDosCampi } = require('../services/campusAgenda');
...
router.get('/cron/gerar-cultos-recorrentes', async (req, res) => {
  ...
  try {
    const resultado = await gerarAgendaDosCampi({ supabase });
    console.log(`[integracao/cron/gerar-cultos] ${resultado.inicio}→${resultado.ate} · ${resultado.criados} criados · ${resultado.campi.length} campi`);
    return res.status(resultado.ok ? 200 : 503).json(resultado);
  } catch (e) {
    console.error('[integracao/cron/gerar-cultos]', e.message);
    return res.status(503).json({ error: 'Não foi possível gerar os cultos recorrentes.' });
  }
});
