...
- **Limpeza pós-auditoria (2026-05-31):** ✅ Pedro (`habilidade='coordenador'`) **fora das raias** do Planner e do cálculo DEM-CAP (`.neq('habilidade','coordenador')`); ✅ **Admin edita `slots_dia`** no membro (não mais horas) — POST/PATCH `/admin/membros` aceitam `slots_dia`; ✅ api.js limpo dos mortos (`capacidade`, `estimar`, `fila.list/reordenar`, `decidirUrgencia` · `fila.posicao` fica). A migration de **DROP** (`fn_marketing_calcular_capacidade_semana`, `fn_marketing_estimar_prazo`, tabela `marketing_grupo_padrao`) + remoção dos endpoints backend mortos vão num PR à parte (tem migration).
