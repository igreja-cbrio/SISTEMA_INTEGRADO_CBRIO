// PAINEL DOS VISITANTES · quem lê a porta pública /visitante.
//
// ⚠️⚠️ NÃO É MÓDULO (decisão do Marcos, 11/09/2026: "não queria isso"). O item
// saiu do menu e da busca; o visitante vive em **Cuidados → Próximos passos**,
// com a etiqueta e a nota da pesquisa na ficha. Este painel é a parte
// OPERACIONAL que sobrou e é montado em dois lugares:
//   · Cuidados → aba "Visitantes" (equipe · embutido, sem cabeçalho próprio)
//   · /visitantes (rota sem menu) — o balcão da CAFETERIA, que tem o módulo
//     `visitantes` e NÃO pode ter `cuidados` (lá está a fila pastoral).
//     É por isso que o módulo de permissão continua existindo.
//
...
/**
 * `embutido` = montado dentro de outra tela (Cuidados). Muda DUAS coisas:
 * some o cabeçalho de módulo, e a aba passa a viver em estado LOCAL.
 * ⚠️ O estado local é obrigatório no modo embutido: o Cuidados já usa `?tab=`
 * pra as abas DELE, e `setParams({ tab })` aqui sobrescreveria a navegação
 * da tela inteira (além de apagar os outros parâmetros da URL).
 */
export default function PainelVisitantes({ embutido = false }: { embutido?: boolean }) {
  ...
  // Quem resgata café: nível 2 em visitantes (a cafeteria) OU quem já cuida
  // da fila (a equipe que abre isto de dentro do Cuidados).
