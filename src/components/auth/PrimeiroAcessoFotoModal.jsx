...
export default function PrimeiroAcessoFotoModal() {
  ...
  async function aoEscolherArquivo(e) {
    ...
    setEnviando(true);
    setStatus(ehHeic ? 'Convertendo foto do iPhone...' : 'Preparando foto...');
    try {
      const pronto = await processarImagemPerfil(file, {
        onProgress: (etapa) => {
          if (etapa === 'convertendo') setStatus('Convertendo foto do iPhone...');
          else if (etapa === 'comprimindo') setStatus('Otimizando foto...');
        },
      });
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) dispensar(); }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            ...
            <DialogTitle>Deixe o sistema com a sua cara</DialogTitle>
          </div>
