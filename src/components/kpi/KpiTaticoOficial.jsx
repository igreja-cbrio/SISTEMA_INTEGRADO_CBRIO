...
export default function KpiTaticoOficial({ fetchFn, titulo = 'KPI tático oficial' }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ fontSize: 11.5, color: 'var(--cbrio-text3)', display: 'flex', alignItems: 'center', gap: 6 }}>
        <BarChart3 size={12} />
        ...
        {state.kpis.map(k => {
          ...
          return (
            <div key={k.id} style={{
              ...
              <div style={{ fontSize: 12.5, fontWeight: 600, marginBottom: 4 }}>{k.indicador}</div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                <span style={{ fontSize: 20, fontWeight: 700, color: semDado ? 'var(--cbrio-text3)' : corStatus(t?.status_trajetoria) }}>
                  {semDado ? '—' : `${t.ultimo_valor}${k.unidade === '%' ? '%' : ''}`}
                </span>
                {k.meta_valor != null && (
                  <span style={{ fontSize: 11, color: 'var(--cbrio-text3)' }}>meta {k.meta_valor}{k.unidade === '%' ? '%' : ''}</span>
                )}
              </div>
              <div style={{ fontSize: 10.5, color: 'var(--cbrio-text3)', marginTop: 2 }}>
